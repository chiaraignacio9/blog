<nav class="bg-gray-800" x-data="{ open: false }">
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              ...
              @foreach ($categories as $category)
                <a href="{{ route('post.category', $category) }}" class="bg-gray-900 text-white px-3 py-2 rounded-md text-sm font-medium" aria-current="page">{{ $category->name }}</a>
              @endforeach
            </div>
          </div>
        </div>
    ...
    <div class="sm:hidden" id="mobile-menu" x-show="open" x-on:click.away="open = false">
      <div class="space-y-1 px-2 pt-2 pb-3">
        ...
        @foreach ($categories as $category)
            <a href="{{ route('post.category', $category) }}" class="bg-gray-900 text-white block px-3 py-2 rounded-md text-base font-medium" aria-current="page">{{ $category->name }}</a>
        @endforeach
